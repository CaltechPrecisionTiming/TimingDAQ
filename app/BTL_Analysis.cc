...
int main( int argc, char** argv)
{
  ...
  bool calibrateChannels = false;
  std::string s_calibrateChannels = ParseCommandLine( argc, argv, "--calibrateChannels" );

  if ( s_calibrateChannels == "true" || s_calibrateChannels == "True")
    calibrateChannels = true;
  else if ( s_calibrateChannels == "false" || s_calibrateChannels == "False" || s_calibrateChannels == "") 
    calibrateChannels = false;
  else {
    std::cerr << "[ERROR]: Please use --calibrateChannels=<true/false> . Running program without specifying option defaults to false\n";
    return -1;
  }
  ...
  if( calibrateChannels ) { // calibration run
    // Calibrate Ratios for one channel
    myPulse->CalibrateOneChannel(1);
    myPulse->CalibrateOneChannel(2);

  }
  else { // analysis run
    // Check performance of one DRS channel. Few simple distributoins
    myPulse->SimpleCheck(1, "ch1");
    TTree* t_chA = (TTree*)myPulse->GetOutputTree();
    myPulse->SimpleCheck(2, "ch2");
