...
if __name__ == '__main__':
    ...
    chan_list[19] = 'ch 19: SP W1'
    chan_list[20] = 'ch 20: SP W4'
    chan_list[21] = 'ch 21: LP W5'
    chan_list[22] = 'ch 22: ---'
    chan_list[23] = 'ch 23: HPK 0'
    chan_list[24] = 'ch 24: HPK 1'
    chan_list[25] = 'ch 25: HPK 2'
    chan_list[26] = 'ch 26: Trigger'

    chan_list[27] = 'ch 27: Photek 1'
    chan_list[28] = 'ch 28: Photek 2'
    ...
    can_amp = TCanvas('amp', 'amp', 2000,1000)
    can_amp.Divide(8,4)
    ...
    dut_dict['Ch 19: ETL SP W1'] = [19]
    dut_dict['Ch 20: ETL SP W4'] = [20]
    dut_dict['Ch 21: ETL LP W5'] = [21]
    dut_dict['ch 23: HPK 0'] = [23]
    dut_dict['ch 24: HPK 1'] = [24]
    dut_dict['ch 25: HPK 2'] = [25]
    dut_dict['Ch 29: 2x2 cm trigger'] = [29]

    can_xy = TCanvas("xy", 'xy', 2000,1000)
    can_xy.Divide(5, 3)
    ...
    for ich in range(0,32):
        hxy_amp.append(TH2D('xy_amp100_ch{}'.format(ich),
                            'Ch {}: XY, amp > 50; X [mm]; Y [mm]'.format(ich),
                            50,0,50,50,0,50))
        pulse.Project(hxy_amp[ich].GetName(), 'yIntercept:xIntercept',
                      baseline+'&& amp['+str(ich)+']>'+str(args.min_amp))
        if (hxy_amp[ich].GetEntries()):
            ch_trigs = hxy_amp[ich].GetEntries()
            print 'Ch', ich, 'triggers:', ch_trigs,'eff = ', float(ch_trigs)/tot_trigs 
    # combine channels on the same device and plot
    hxy_dut = []
    ip = 2
    for dut, ch in dut_dict.iteritems():
        ip += 1
        can_xy.cd(ip)
        hxy_amp[ch[0]].SetTitle(dut)
        if len(ch)>1:
            for i in range(1, len(ch)):
                hxy_amp[ch[0]].Add(hxy_amp[ch[i]])
        hxy_amp[ch[0]].DrawCopy('COLZ')
    can_xy.Update()    
    # can_xy.SaveAs(tag+"_xy.root")
    ...
    for ch in range(0,36):
        ...
        hamp.append(TH1D('amp_ch{}'.format(ch),
                         'Ch {}: '+chan_list[ch]+'; Amplitude [mV]'.format(ch),50,0,300))
